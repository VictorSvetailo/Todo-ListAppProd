...
export const Task: FC<TaskPropsType> = React.memo(({todoListId, task}) => {
    ...
    return (
        ...
        <div key={task.id} className={task.status === TaskStatuses.Completed ? 'is_done' : ''}>
            <Box sx={{ width: '100%' }}>
                <Grid container
                      direction="row"
                      alignItems="center"
                      rowSpacing={0} columnSpacing={{ xs: 0.1, sm:  0.1, md: 0.1 }}>
                    <Grid
                        container
                        direction="row"
                        ...
                        item xs={2}>
                        <Checkbox
                            onChange={onChangeStatus} checked={task.status === TaskStatuses.Completed}
                            {...label}
                            sx={{
                                color: '#1977d2',
                                '&.Mui-checked': {
                    ...
                    <Grid
                        ...
                        <IconButton onClick={removeTaskHandler}
                            ...
                            <ClearIcon style={{color: '#fe0100'}} fontSize="small"/>
                        </IconButton>
                    </Grid>
                </Grid>
            </Box>
