   ...
   return (
      <div className={styles.block_list}>
         <h3>
            ...
            <button disabled={props.todoList.entityStatus === "loading"} onClick={removeTodoListHandler}>
               x
            </button>
         </h3>
         <div>
         ...
         <ul>
            {tasksForTodolist.map((task: TaskType) => (
               <Tasks key={task.id} task={task} toDoListID={props.todoList.id} title={props.todoList.title} />
            ))}
         </ul>
         <div>
            <button disabled={props.todoList.entityStatus === "loading"} className={props.todoList.filter === "all" ? `${styles.active_filter}` : ""} onClick={onClickSetFilterCreator("all")}>
               All
            </button>
            <button className={props.todoList.filter === "active" ? `${styles.active_filter}` : ""} onClick={onClickSetFilterCreator("active")}>
               Active
            </button>
            <button className={props.todoList.filter === "completed" ? `${styles.active_filter}` : ""} onClick={onClickSetFilterCreator("completed")}>
               Completed
            </button>
         </div>
      </div>
